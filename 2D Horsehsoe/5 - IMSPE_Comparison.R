...
Num_Integ_Pts <- read.table(paste(wd.aux, "/Horseshoe_Embedded_Integration_", embedding.dim, "Dim_", 
                                grid.density, ".txt", sep = ''))
...
for(i in 1:nrow(Design_List))
{
	...
	size <- des[1]
	rho <- des[2:5]
	design <- read.table(paste(wd.maximin,"/Horseshoe_Size_", size ,"_Embedded_Maximin_Design.txt", 										sep=''))	
			
	opt.design <- read.table(paste(wd.imspe, "/Horseshoe_Size_" ,size ,"_Embedded_IMSPE_Design_", (i-1)%/%3+1, ".txt", 											sep=''))
...
for(i in 1:nrow(Design_List))
{
	...
	size <- des[1]
	rho <- des[2:5]
	design <- read.table(paste(wd.euclid,"/Horseshoe_Size_", size ,"_Embedded_Euclidean_Maximin_Design.txt", 										sep=''))	
...
for(i in 1:nrow(Design_List))
{
	...
	size <- des[1]
	rho <- des[2:5]
	design <- as.matrix(read.table(paste(wd.fw,"/Fast_Ward_Size_", size ,".txt", 										          sep='')))	
	#temp <- Embed_Integ_Pts(design, Orig_Cands, Iso, epsilon) 	
	#design <- temp$Embed_Integ_Pts[,1:4]
	#write.table(design, paste(wd.fw, "/Embedded_Fast_Ward_Size_", size, ".txt", sep=''))
	design <- read.table(paste(wd.fw, "/Embedded_Fast_Ward_Size_", size, ".txt", sep=''))
...
rho <- rbind(c(.9,.9,.9,.9), c(.6,.6,.6,.6), c(.3,.3,.3,.3), c(.9,.3,.9,.9))
sizes <- c(8,12,16)
IMSPE.Iopt <- c()
Eff <- c()
...
for(n in sizes)
{
	for(i in 1:nrow(rho))
	{
		opt.design <- read.table(paste(wd.imspe,"/Horseshoe_Size_" ,n ,"_Embedded_IMSPE_Design_", i, ".txt", sep=''))
		opt.IMSPE <- tempmse(rho[i,],opt.design,Num_Integ_Pts,sigmatau2)
		for(j in 1:nrow(rho))
		{
			design <- read.table(paste(wd.imspe,"/Horseshoe_Size_" ,n ,"_Embedded_IMSPE_Design_", j, ".txt", sep=''))
...
names(IMSPE.all) <- c("n", paste("true_rho",1:l, sep=''), paste("assumed_rho",1:l, sep=''), "IMSPE", "Eff")
IMSPE.all

IMSPE.Eff <- aggregate(IMSPE.all$Eff, by = list(IMSPE.all$n, IMSPE.all$assumed_rho1, IMSPE.all$assumed_rho2, IMSPE.all$assumed_rho3, IMSPE.all$assumed_rho4), FUN = "mean")
IMSPE.Eff$x <- round(IMSPE.Eff$x,3)

names(IMSPE.Eff) <- names(IMSPE.Eff) <- c("n", "rho1", "rho2", "rho3", "rho4", "Efficiency")
IMSPE.Eff