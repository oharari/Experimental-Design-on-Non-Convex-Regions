...
corr.matrix.matern.1D <- function(X, theta)
{
	d <- as.matrix(dist(X))
	R <- (1 + sqrt(5)*d/theta + 5*d^2/(3*theta^2))*exp(-sqrt(5)*d/theta)
...
corr.matrix.matern <- function(X, theta)
{
	n <- dim(X)[1]
	R <- matrix(1, nrow=n, ncol=n)
	for(i in 1:length(theta))
	{
		R <- R*corr.matrix.matern.1D(X[,i], theta[i])
	}
...
cross.corr.matrix.matern.1D <- function(X.new, X.old, theta)
{
	d <- as.matrix(dist(X.old, X.new))
	R <- (1 + sqrt(5)*d/theta + 5*d^2/(3*theta^2))*exp(-sqrt(5)*d/theta)
...
cross.corr.matrix.matern <- function(X.new, X.old, theta)
{
	n1 <- dim(X.old)[1]
	n2 <- dim(X.new)[1]
	
	R <- matrix(1, nrow=n1, ncol=n2)
	for(i in 1:length(theta))
	{
		R <- R*cross.corr.matrix.matern.1D(X.new[,i], X.old[,i], theta[i])
	}
...
tempmse2 <- function(theta,design,Embed_Integ_Pts,sigmatau2=1e-5)
{
	  N=nrow(design)
      one=rep(1,N)
      R=corr.matrix.matern(design,theta)
			    # IMSE about 8% of the time for rho=.99, ss=10.  This would
			    # dissapear around 1e-7.  I set it to 1e-5 and it seems
			    # to work all the way to rho=.99999999, ss=10.  Hopefully
			    # will also be sufficient for larger designs (ss>10).
	    ...
	    Rt <- t(cross.corr.matrix.matern(Embed_Integ_Pts, design, theta))
...
imsedesign2 <- function(Init_Design, Embed_Cands, Embed_Integ_Pts, theta, sigmatau2=1e-5,
                       hshindex=NULL, hashlist=NULL, imselist=NULL, maxhash=16000, maxit)
{
	numcandidates=nrow(Embed_Cands)
	N=nrow(Init_Design)
                               ...
                               sigmatau2=1e-5, maxit)
{
	...
	for(i in 1:N.Iter) 
	{
 		d=imsedesign2(Init_Design,Embed_Cands,Embed_Integ_Pts,theta,sigmatau2,maxit=maxit)
...
Non_Convex_IMSPE_Optim_Design2 <- function(Init_Design, Embed_Cands,theta,Embed_Integ_Pts,
										  N.Iter, Orig_Cands, sigmatau2=1e-5, maxit)
{
	imspe.obj <- IMSPE_Optim_Design2(Init_Design,Embed_Cands,theta,Embed_Integ_Pts,N.Iter,maxit=maxit)
	rows <- rownames(imspe.obj$design)
	Non.Conv.Design <- Orig_Cands[rows,]
	IMSPE <- imspe.obj$IMSPE
...
Non_Convex_IMSPE_Optim_Estimated <- function(X,y,Init_Design, Embed_Cands,Embed_Integ_Pts,
										     N.Iter, Orig_Cands, maxit)
{
	mod <- km(design=X, response=y, covtype="matern5_2", nugget.estim=TRUE, 
                control = list(trace=FALSE))
	theta <- (mod@covariance)@range.val
	sigmatau2 <- (mod@covariance)@nugget
	imspe.obj <- IMSPE_Optim_Design2(Init_Design,Embed_Cands,theta,Embed_Integ_Pts,N.Iter,
						   sigmatau2=sigmatau2, maxit=maxit)
	IMSPE <- imspe.obj$IMSPE
