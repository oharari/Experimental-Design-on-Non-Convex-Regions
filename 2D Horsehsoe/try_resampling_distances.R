#*******************************************************************************
#* This code creates some useful tables for later use in the Annular Ring 
#* Example
#*******************************************************************************
...
candidates = read.table(paste("Horseshoe_Candidates_", as.character(epsilon), 
                              ".txt", sep = ''))
#plot(candidates)
...
ma = Iso$EmbedPts
GeoDist = Iso$GeoDist
write.table(ma, paste(wd.write, "/Horseshoe_Embedded_Candidates_Full_", 
                      as.character(epsilon), ".txt", sep = ''))
write.table(GeoDist, paste(wd.write, "/Horseshoe_Geodesic_Distances_", 
                           as.character(epsilon), ".txt", sep = ''))


#Focusing on the first 4 dimensions of the embedding (and writing to file)
ma = Iso$EmbedPts[, 1:numapproxevals]
write.table(ma, paste(wd.write, "/Horseshoe_Embedded_Candidates_", 
                      as.character(numapproxevals), "Dim_", 
                      as.character(epsilon), ".txt", sep = ''))
...
write.table(Emb_Integ$design, 
            paste(wd.write, "/Horseshoe_Embedded_Integration_", 
                  as.character(numapproxevals), "Dim_", as.character(epsilon), 
                  ".txt", sep = ''))
