#*******************************************************************************
#* This function calculates the correlation matrix at a design matrix based on 
#* the sqaured-exponential correlation function of an isotropic Gaussian process           
#*
...
#*******************************************************************************
#* This function calculates the cross-correlation matrix between a design and a 
#* new set of points																		
#*																				
#* is called by: tempmse; 																
...
#*******************************************************************************
#* This function calculates the Integrated Mean-Squared Prediction Error at a 
#* design 
#*
#* calls: corr.matrix; cross.corr.matrix;                                     
#* is called by: imsedesign; 															
#* arguments: rho - a correlation parameter; design - a design matrix; 			
#* 				    Embed_Integ_Pts - embedded points for numerical integration;	
...
tempmse = function(rho,design,Embed_Integ_Pts,sigmatau2=1e-5){
  ...
  # disappear around 1e-7.  I set it to 1e-5 and it seems
...
#*******************************************************************************
#* This function finds an (approximate) IMSPE-optimal design - in the 
#* alternative (embedded) coordinate system - using row exchanges with an 
#* hashing algorithm to spare repeated comparisons of similar designs                                    
#* 							                            							
...
#*******************************************************************************
imsedesign = function(Init_Design, Embed_Cands, Embed_Integ_Pts, rho, 
                      sigmatau2=1e-5, hshindex=NULL, hashlist=NULL, 
                      imselist=NULL, maxhash=16000){
...
imsedesign2 = function(Init_Design, Embed_Cands, Embed_Integ_Pts, theta, 
                       sigmatau2=1e-5, hshindex=NULL, hashlist=NULL, 
                       imselist=NULL, maxhash=16000){
  numcandidates = nrow(Embed_Cands)
  N = nrow(Init_Design)
...
#*******************************************************************************
#* This function calls "imsedesigns" potentially multiple times, and retains the 
#* best design                                   											
#* 							                            							
#* calls: imsedesign;                                                                  
...
#*******************************************************************************
IMSPE_Optim_Design = function(Init_Design, Embed_Cands, rho,Embed_Integ_Pts, 
                              N.Iter, sigmatau2=1e-5){
...
IMSPE_Optim_Design2 = function(Init_Design, Embed_Cands, theta, Embed_Integ_Pts, 
                               N.Iter, sigmatau2=1e-5){
  imsevals = rep(0, N.Iter)
  bestimspe = 1e50
...
#*******************************************************************************
#* This function calls translates the design obtained by "IMSPE_Optim_Design" 
#* back to the original Cartesian coordinates												
#* 							                            							
...
#* values: design - the chosen design; IMSPE - the IMSPE of the chosen design		 	
#*******************************************************************************
Non_Convex_IMSPE_Optim_Design = function(Init_Design, Embed_Cands, rho,
                                         Embed_Integ_Pts, N.Iter, Orig_Cands, 
                                         sigmatau2=1e-5){
  imspe.obj = IMSPE_Optim_Design(Init_Design, Embed_Cands, rho,Embed_Integ_Pts,
                                 N.Iter)
...
Non_Convex_IMSPE_Optim_Design2 = function(Init_Design, Embed_Cands,theta,
                                          Embed_Integ_Pts, N.Iter, Orig_Cands, 
                                          sigmatau2=1e-5){
  imspe.obj = IMSPE_Optim_Design2(Init_Design,Embed_Cands, 
                                  theta,Embed_Integ_Pts, N.Iter)
  rows = rownames(imspe.obj$design)
  Non.Conv.Design = Orig_Cands[rows,]
  IMSPE = imspe.obj$IMSPE
...
Non_Convex_IMSPE_Optim_Estimated = function(X, y, Init_Design, Embed_Cands,
                                            Embed_Integ_Pts, N.Iter, 
                                            Orig_Cands){
  ...
  imspe.obj = IMSPE_Optim_Design2(Init_Design, Embed_Cands, theta, 
                                  Embed_Integ_Pts, N.Iter, sigmatau2)
  IMSPE = imspe.obj$IMSPE
