...
GeoDist = as.matrix(read.table(paste(wd.read, "/Horseshoe_Geodesic_Distances_", 
                                     epsilon,".txt", sep = '')))

#** generating a design on size *design.size*
D = MaximinDesign(cands, design.size, GeoDist, nruns = 10, 
                  max.loop = 500)$design


#** plotting the resultant design
tit = substitute(paste('Size ', nn, ' Maximin Design', sep = ''), 
                 list(nn=design.size))
pdf(paste(wd.write, "/Horseshoe_Size_", design.size, "_Maximin_Design.pdf", 
          sep = ''), width = 6.2, height = 6)
par(mar=c(5,5,3,3))
