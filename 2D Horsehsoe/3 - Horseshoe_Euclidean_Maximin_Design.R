#*******************************************************************************
#* Before proceeding, make sure the following packages are installed on your 
#* machine: 
#*******************************************************************************
...
for(i in 1:length(sizes)){
  ...
  D = MaximinDesign(cands, design.size, Dist, nruns = 10, 
                    max.loop = 1000)$design
  
  tit = substitute(paste('Size ', nn, ' Euclidean Maximin Design', sep = ''), 
                   list(nn=design.size))
  pdf(paste(wd.write, "/Horseshoe_Size_", design.size, 
            "_Euclidean_Maximin_Design.pdf", sep = ''), width = 6.2, height = 6)
  
  par(mar=c(5,5,3,3))
  plot.cands = as.matrix(expand.grid(seq(0,1,.05), seq(0,1,.05)))
  hs = horseshoe(plot.cands, epsilon)$newcands
  ...
  write.table(D, paste(wd.write, "/Horseshoe_Size_", design.size, 
                       "_Euclidean_Maximin_Design.txt", sep = ''))
  
  write.table(cands.embedded[rownames(D),], 
              paste(wd.write, "/Horseshoe_Size_", design.size, 
                    "_Embedded_Euclidean_Maximin_Design.txt", 
                    sep = ''))
}
